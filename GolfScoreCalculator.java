    ...
    void calculateScores(int[] pars) {
        ...
        for (int i = 0; i < pars.length; i++) {
            ...
            if (scores[i] > 2 * pars[i]) {
                totalAdjustedScore += 2 * pars[i]; // Adjust score if greater than double par
            } else {
                totalAdjustedScore += scores[i];
            }
        }
        // Simple handicap calculation (adjust as needed)
...
public class GolfScoreCalculator {
    public static void main(String[] args) {
        ...
        // Define courses
        GolfCourse[] courses = {
                new GolfCourse("Abdulrachman Saleh Golf Course", new int[]{4, 4, 3, 5, 4, 4, 3, 4, 5, 5, 3, 4, 4, 4, 4, 3, 4, 5}),
                new GolfCourse("The Araya Golf Course", new int[]{4, 4, 4, 3, 5, 3, 4, 5, 4, 5, 5, 4, 4, 3, 4, 3, 4, 4})
        };
